...
if(isset($_POST['login_btn'])){
  ...
  $admin_stmt = $conn->prepare("SELECT admin_id, admin_name, admin_email, admin_password FROM admins WHERE admin_email = ? AND admin_password = ? LIMIT 1");
  $admin_stmt->bind_param('ss',$email,$password);
  
  if($admin_stmt->execute()){
    $admin_stmt->bind_result($admin_id,$admin_name,$admin_email,$admin_password);
